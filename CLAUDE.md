...
## LIVE STATE (2026-07-18 Session 10 · T2 verified via aethermind MCP + live endpoint probes)
CONFIRMED (DB / storage / API, verified this session):
- Site: aethermind-five.vercel.app. DB: 118 questions (010 applied, 120 to 118), 12 realms. Leaderboard am_scores: 1 real row (mahil, xp 350 / level 8 / max_streak 15 after the Session 10 health check); test/probe rows come and go, so delete any non-mahil row before the Austin launch.
...
OPUS BROWSER AUDIT (2026-07-17): 63/100. Biggest gaps: social sharing 2/10, accessibility 5/10. Known data gaps: image_url is NULL for all 120 (client falls back to image_search stock photos); correct_idx STILL ships to the client (NOT stripped as of Session 10), so the leaderboard remains cheatable. SECURITY DECISION (Mahil, Session 10): for the Karachi friend-group threat model honor-system is ACCEPTED; full server-side answer-session state is DEFERRED (not scheduled). The correct_idx STRIP is approved to proceed once T1 signals BOTH QuizScreen AND DailyAether are wired to server grading via checkAnswer (T1 wiring in progress). BLOCKER for the strip (flagged to T1): stripping correct_idx makes q.correct_index undefined, but the store's answerQuestion grades on it (ok = idx===q.correct_index) and builds the learning card from it, so answerQuestion needs an optional serverResult {isCorrect, correctIdx} param FIRST (additive, inert until passed), shipped in lockstep with T1's wiring and the strip. DailyAether also grades client-side on correct_index, so it must move to checkAnswer as part of the same cutover. Not stripped until all three land together.
