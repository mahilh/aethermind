...
## LIVE STATE (2026-07-18 Session 16 · T2 verified via aethermind MCP + live endpoint probes)
CONFIRMED (DB / storage / API, verified this session):
- Site: aethermind-five.vercel.app. DB: 118 questions (010 applied, 120 to 118) across 12 realms: 11 realms have 10, Ethics & Wisdom has 8 (both 010 dedup removals were there). Daily Aether (1 from each of 5 realms) pool is healthy (min realm 8 > 5). NEW TABLE am_suggestions (Session 16): community knowledge suggestions (type book/topic/teacher/tradition/other, title, description, realm_name, upvotes, status pending/reviewed/implemented). anon SELECT+INSERT via RLS; written through /api/suggest (service_role, validated, rate-limited 20/hr/IP). 1 seed row 'The Kybalion'. This is how the question bank grows past 118. Leaderboard am_scores (Session 16): mahil ONLY (xp 540 / level 10 / max_streak 18). The '🧠⚡🌙' emoji row (Mahil confirmed it was their own test), plus 'Test player' and 'AuditPlayer' probes, were deleted via the Management API query endpoint (specific-name IN list, NEVER a blanket non-mahil DELETE). Test/probe rows RECUR while a Playwright audit runs, so re-sweep specific names right before the Austin launch. NOTE: the aethermind MCP get_leaderboard/check_db UNDER-REPORT rows (showed 1 when REST showed 2-3); trust direct anon REST for the authoritative set.
...
T2 SESSION 14 (CRITICAL seed-data fix): all 118 questions had correct_idx=0 (correct answer was always option A), so "always tap A" scored 100%. FIXED by scripts/shuffle-options.js: a deterministic per-question-id Park-Miller shuffle of the options with correct_idx updated to follow the correct answer. Dry-run by default with per-question sanity asserts (correct-answer text preserved + true permutation); applied Session 14, verified live: distribution now {0:25,1:40,2:28,3:25}, double-slit correct answer preserved at its new index, save-score still {"ok":true}. CAVEAT: this kills the trivial always-A exploit but correct_idx STILL ships to the client, so a DevTools reader can still cheat, that needs the HELD checkAnswer/server-scoring fix. It is a JS script (not a numbered SQL migration) because the seeded shuffle is JS, and is NOT idempotent (re-running --apply re-shuffles but preserves correctness). Fixed the boot script's Math.imul negative-index bug + ported to ESM. T1 Session 13: quantumShimmer softened (1cca44e), BEST STREAK CharacterSheet card (87277d4), Blind Seer purple + Speed TIME popup (f614b6a).
T2 SESSION 15 (cleanup + launch prep): fixed the pre-existing em-dash in scripts/run-migrations.js (0c7d21e); added LAUNCH_CHECKLIST.md (4bd0de9) at repo root. VERIFIED the shuffle reached every realm: all 12 realms have >=3 distinct correct_idx (none all-A); the index-1 skew is a Park-Miller artifact with these UUIDs (~1.4 SD, acceptable, not exploitable). Health suite GREEN: save INSERT/UPDATE ok, GREATEST held (max_streak stayed 18 after a streak:3 write), cheat xp:99999/L2 -> Implausible, check-answer 400/404/405 correct, daily-questions 200. CONTENT DECISION: generic-noun possessives + stripped serial commas remain; a regex fix (Option A) is UNSAFE (would corrupt "its own" -> "it's own", confirmed 2 such rows), so Option B (leave as-is, 014 fixed the visible famous names) NOW and Option C (AI regeneration, needs Mahil's model/budget/scope call) as the clean fix. NOTE: run-migrations.js uses CommonJS require but the project is ESM, so it will NOT run as-is (needs .cjs or import); the Management API query endpoint is the working DB-write path used this session. mahil L10/xp530/streak18. T1 Session 14: HomeScreen name input Cinzel + esoteric placeholder, no "(optional)" (c933159); Speed number amber + Gauntlet counter 9px (cadd0dc).
T2 SESSION 16 (suggestions system + ESM fix): scripts/run-migrations.js CONVERTED to ESM (fe862b4, import pg; was CommonJS require, would throw at runtime; verified it now reaches the AETHERMIND_DB check). NEW api/suggest.js (c440fe9): POST {type,title,description,realmName,playerName} -> am_suggestions; hardened beyond the boot spec with a per-IP 20/hr rate limit, env guard, realm-name whitelist, and try/catch (the spec had none of these). Migration 016 record written (e2e0a46, verified matches the live table + RLS policies). FastMCP get_suggestions(status) + top_suggestions(limit) added (014e987, sync, reuse rest_get); callable in a fresh MCP session. LIVE-VERIFIED: /api/suggest saves valid input {"ok":true}, rejects a bad type and a short title with 400; The Kybalion row present. CAVEAT: am_suggestions allows anon INSERT directly via PostgREST (public anon key), so the endpoint's rate-limit + validation only bind app clients; the DB CHECK constraints (type/title/description) bind even direct inserts. mahil L10/xp540/streak18. T1 Session 16: GameOver shake+heartbeat (042dd1e), Gauntlet gold rings (d7f0eb9), WisdomVault fadeInUp (df10b16), ModeSelect taglines (7ccd0e1).
...
api/suggest.js: POST community knowledge suggestions to am_suggestions (validated, rate-limited 20/hr/IP)
