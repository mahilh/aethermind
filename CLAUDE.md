...
- Site: aethermind-five.vercel.app. DB: 120 questions, 12 realms (10 each). Leaderboard am_scores: 9 rows, all test/probe entries except possibly 'mahil' (one row is an XSS probe '<script>...'); migration 008 written to delete them, run it in the SQL Editor before the Austin launch.
...
OPUS BROWSER AUDIT (2026-07-17): 63/100. Biggest gaps: social sharing 2/10, accessibility 5/10. Known data gaps: image_url is NULL for all 120 (client falls back to image_search stock photos); correct_idx ships to the client, so answers are readable in DevTools and the leaderboard is trivially cheatable (needs server-side answer checking, a larger change deferred). Daily Aether (shared daily questions + Wordle-style emoji share): T1 implementing.
