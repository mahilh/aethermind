...
OPUS BROWSER AUDIT (2026-07-17): 63/100. Biggest gaps: social sharing 2/10, accessibility 5/10. Known data gaps: image_url is NULL for all 120 (client falls back to image_search stock photos); correct_idx STILL ships to the client (NOT stripped this session), so the leaderboard remains cheatable. check-answer is now throttled (Session 9) but that does NOT close the hole: it returns correctIdx on any pick, so a cheater can call it once before answering. The real fix (server-side per-question session state so correctIdx is never returned before an answer is locked, which also means routing the question fetch through a server endpoint) is the next JOINT T1+T2 task; checkAnswer wiring + the correct_idx strip stay HELD until then. Daily Aether (shared daily questions + Wordle-style emoji share): T1 implementing.
